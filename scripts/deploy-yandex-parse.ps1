...
$FunctionName = if ($env:YC_PARSE_FUNCTION_NAME) { $env:YC_PARSE_FUNCTION_NAME } else { 'avers-parse' }
...
try {
  Copy-Item (Join-Path $Root 'yandex-cloud\parseEstimate\index.js') $StageDir
  Copy-Item (Join-Path $Root 'yandex-cloud\parseEstimate\package.json') $StageDir
  Copy-Item (Join-Path $Root 'functions\yandexGpt.js') (Join-Path $StageDir 'yandexGpt.js')
  ...
  Write-Host ''
  Write-Host 'Then restart dev server or rebuild: npm run deploy'
}
finally {
